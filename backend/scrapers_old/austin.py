...
class AustinPermitScraper:
    def __init__(self):
        ...
        # Firebase init - initialize if not already done
    ...
    def fetch_permits(self):
        """Fetch permits from Austin's open data API"""
        print("🏗️  Austin TX Construction Permits Scraper")
        print("=" * 50)
    ...
    def generate_mock_permits(self):
        ...
        streets = ['Congress Ave', '6th St', 'Guadalupe St', 'Lamar Blvd', 'Burnet Rd', 'South Congress Ave']
        ...
        for i in range(5000):
            ...
            permit = {
                'permit_number': f'AUS{datetime.now().strftime("%y%m")}{10000 + i:05d}',
                'issue_date': issue_date,
                'work_type': random.choice(work_types),
                'project_name': f'Austin Project {i+1}',
                'description': f'Construction work at {random.randint(100, 9999)} {random.choice(streets)}',
                'address': f'{random.randint(100, 9999)} {random.choice(streets)}',
                'city': 'Austin',
                'zip_code': f'787{random.randint(10, 99):02d}',
                'owner_name': f'Austin Property Owner {i+1}',
                'contractor': f'Austin Builders LLC {i%10 + 1}',
                'contractor_phone': f'(512) {random.randint(200, 999):03d}-{random.randint(1000, 9999):04d}',
